...
class TestSendSolveNotification:
    ...
    async def test_stream_channel_not_found_no_action(
        ...
        with patch(
            "modules.hydra.hydra_utils.old_lion_command_helpers.google_utils"
        ) as mock_google_utils:
            await send_solve_notification(mock_bot, mock_ctx, "TEST")
    ...
    async def test_big_board_missing_columns_counts_zero_first_solve_message(
        ...
        with patch(
            "modules.hydra.hydra_utils.old_lion_command_helpers.discord_utils"
    ...
    async def test_big_board_gspread_api_error_treated_as_zero(
        ...
        monkeypatch.setenv("TM_BOT_STREAM_CHANNEL_ID", "123456")
        monkeypatch.setenv(
            "TM_BIG_BOARD_URL", "https://docs.google.com/spreadsheets/d/test"
        )
    ...
    async def test_integration_with_get_ordinal_suffix_various(
        ...
        for count, expected_suffix in test_cases:
            ...
            with (
                ...
                mock_google_utils.create_gspread_client.return_value = (
                    mock_gspread_client
                )
                ...
                call_args = mock_embed.add_field.call_args
                assert expected_suffix in call_args[1]["value"], (
                    f"Expected {expected_suffix} for count {count}"
                )
