...
class PerfectPitch(commands.Cog, name="Perfect Pitch"):
    ...
    async def playtunecustom(self, ctx):
        ...
        embed.add_field(
            name=f"Customizing Note Length",
    ...
    async def playtuneinstrument(self, ctx):
        ...
        logging_utils.log_command(
            "playtuneinstrument", ctx.guild, ctx.channel, ctx.author
        )
        embed = discord_utils.create_embed()

        embed.add_field(
            name="Instruments and Ranges (Low/High)",
        ...
        chord = random.choice(
            glob.glob(
                os.path.join(
                    ...
                    perfect_pitch_constants.CHORDS,
                    "*.mp3",
                )
            )
        )
    ...
    async def note(self, ctx, *args):
        ...
        if octave is not None and (octave < 0 or octave > 7):
            embed.add_field(
                name="Failed!", value="Make sure your octave value is between 0 and 7!"
            )
            await ctx.send(embed=embed)
            return
        ...
        await ctx.send(
            f"Answer: ||{notepath.split('/')[-1].split('.')[0].center(10)}||"
        )
