...
class LionCog(commands.Cog, name="Lion"):
    ...
    @command_predicates.is_solver()
    @commands.command(name="roundlion")
    ...
    @command_predicates.is_solver()
    @commands.command(name="noteslion")
    async def noteslion(self, ctx: commands.Context, *, notes: str):
        ...
        Permission Category : Solver Roles only
        Usage: ~noteslion "Notes about the puzzle"
        """

        await logging_utils.log_command("noteslion", ctx.guild, ctx.channel, ctx.author)
        embed = discord_utils.create_embed()
