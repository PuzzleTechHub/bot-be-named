...
class HydraCog(commands.Cog, name="Hydra"):
    ...
    async def catsummaryhydra(self, ctx, *cat_names) -> None:
        """Summarizes all the text channels in the specified categories, according to the "Notes" column in the tethered GSheet.

        Permission Category : Solver Roles only.

        Usage: `~catsummaryhydra`
        Usage: `~catsummaryhydra "Cat Name"`
        Usage: `~catsummaryhydra "Cat 1" "Cat 2" "Cat 3"`
        """
        await logging_utils.log_command(
        ...
        # Determine which categories to summarize
        categories = []
        failed_categories = []

        if not cat_names:
            categories.append(ctx.channel.category)
        else:
        ...
        if failed_categories:
            embed.add_field(
                name=f"{constants.FAILED}",
                value=f"The following categories were not found:\n"
                f"{', '.join(f'`{name}`' for name in failed_categories)}\n"
                "Will proceed with the found categories.",
                inline=False,
            )
            await discord_utils.send_message(ctx, embed)

        # Exit if no valid categories
        if not categories:
            return
        ...
        start_embed.add_field(
            ...
            value=f"Summarizing categor{'y' if len(categories) == 1 else 'ies'}: "
            f"{', '.join(f'`{cat.name}`' for cat in categories)}. This may take a while.",
            inline=False,
        )
        ...
        try:
            ...
            # Collect all channels and their tethered sheets
            sheet_to_channels: dict[str, list[tuple[str, nextcord.TextChannel]]] = (
                {}
            )  # {sheet_link: [(cat_name, channel)]}

            # Group all channels sharing the same tethered sheet
                    ...
                    if result is None:
                        all_messages.append(
                            (
                                currcat.name,
                                currchan,
                                f"- {currchan.mention} - **No sheet tethered!**",
                            )
                        )
                        continue

                    curr_sheet_link = result.sheet_link
                    if curr_sheet_link not in sheet_to_channels:
                        sheet_to_channels[curr_sheet_link] = []
                    sheet_to_channels[curr_sheet_link].append((currcat.name, currchan))

            # Run all sheet processing in parallel
            tasks = [
                hydra_utils.process_sheet(
                    self.gspread_client, ctx, sheet_link, channels
                )
                for sheet_link, channels in sheet_to_channels.items()
            ]
            ...
            embed.add_field(
                name=f"{constants.FAILED}",
                value=f"An error occurred while summarizing categories. "
                f"Error: {str(e)}",
                inline=False,
            )
            await discord_utils.send_message(ctx, embed)
        ...
        if not all_messages:
            ...
            embed.add_field(
                name=f"{constants.FAILED}",
                value=f"No text channels found in the specified categor{'y' if len(categories) == 1 else 'ies'}.",
                inline=False,
            )
            await discord_utils.send_message(ctx, embed)
            await initial_message.delete()
        ...
        for cat_name, _, msg in all_messages:
            if cat_name not in messages_by_category:
                messages_by_category[cat_name] = []
            messages_by_category[cat_name].append(msg)
        ...
        for cat_name in [cat.name for cat in categories]:
            ...
            messages = messages_by_category[cat_name]
            ...
            for i in range(0, len(messages), chunk_size):
                ...
                final_embed.title = f"Category: {cat_name}"
                ...
                for j, msg in enumerate(chunk):
                    ...
                    final_embed.add_field(
                        name=channel_part, value=desc_part, inline=True
                    )
