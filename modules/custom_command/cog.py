...
class CustomCommandCog(commands.Cog, name="Custom Command"):
    ...
    async def addembedcommand(self, ctx, command_name: str, *args):
        ...
        with Session(database.DATABASE_ENGINE) as session:
            result = session.query(database.CustomCommands)\
                            ...
                            .first()
            if result is None:
                stmt = insert(database.CustomCommands).values(server_id=ctx.guild.id, server_name=ctx.guild.name,
            ...
            database.CUSTOM_COMMANDS[ctx.guild.id][command_name] = (command_return, False)
    ...
    async def addtextcommand(self, ctx, command_name: str, *args):
        ...
        with Session(database.DATABASE_ENGINE) as session:
            ...
            # Command exists in the DB but not in our constants.
                ...
                command_return = result.command_return
            # update constants dict
            database.CUSTOM_COMMANDS[ctx.guild.id][command_name] = (command_return, True)
    ...
    async def lscustomcommands(self, ctx):
        ...
        if ctx.guild.id in database.CUSTOM_COMMANDS and len(database.CUSTOM_COMMANDS[ctx.guild.id]) > 0:
        ...
        if command_name in database.CUSTOM_COMMANDS[ctx.guild.id]:
            ...
            embed.add_field(name=f"{constants.SUCCESS}",
                            value=f"Edited command `{ctx.prefix}{command_name}` to have return value "
                                  f"`{command_return}`")
            database.CUSTOM_COMMANDS[ctx.guild.id][command_name] = (command_return, database.CUSTOM_COMMANDS[ctx.guild.id][command_name][1])
        else:
