...
class SheetsCog(commands.Cog, name="Sheets"):
    ...
    @command_predicates.is_solver()
    @commands.command(name='createpuzzle',aliases=['puzzle'])
    async def create_puzzle(self, ctx, channel_name : str, puzzle_url : str, template_name : str = 'Template'):
        logging_utils.log_command('createpuzzle',ctx.guild,ctx.channel,ctx.author)
